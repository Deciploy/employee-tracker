...
export interface TerminatedForegroundActivity
  extends RunningForegroundActivity {
  endTime: string;
}
...
export class ForegroundTracker {
  ...
  private _onActivitiesStartCallback: (
    activities: Array<RunningForegroundActivity>,
  ) => void;
  private _onActivitiesEndCallback: (
    activities: Array<RunningForegroundActivity>,
  ) => void;
  ...
  private async getRunningActivities(): Promise<
    Array<RunningForegroundActivity>
  > {
    const cmd =
    ...
    return await new Promise((resolve, reject) => {
      exec(cmd, (error, stdout, stderr) => {
        if (error) {
          ...
          const snapshot = JSON.parse(stdout).map(
            (activity: any) =>
              ({
                pid: activity.Id,
                name: activity.Name,
  ...
  private getTerminatedActivities(
    snapshot: Array<RunningForegroundActivity>,
  ): Array<TerminatedForegroundActivity> {
    return this._runningActivity
      .filter(
        (activity) =>
          !snapshot.some(
            (snapshotActivity) => snapshotActivity.pid === activity.pid,
          ),
      )
      .map(
        (activity) =>
          ...
          }) as TerminatedForegroundActivity,
      );
  }

  private getNewActivities(
    snapshot: Array<RunningForegroundActivity>,
  ): Array<RunningForegroundActivity> {
    return snapshot.filter(
      (snapshotActivity) =>
        !this._runningActivity.some(
          (activity) => activity.pid === snapshotActivity.pid,
        ),
    );
  }
